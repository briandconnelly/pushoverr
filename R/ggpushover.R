#' Send a message with plot as picture attachment using Pushover
#'
#' \code{ggpushover} sends a message (push notification) to a user or group with ggplot object as picture attachment.
...
#' @param plot name of ggplot object to plot (default: last_plot())
...
#' @examples
#' \dontrun{
#' ggpushover(message = "Hola Mundo!", plot = last_plot())
#' }
ggpushover <- function(message,
                     title = NULL,
                     priority = 0,
    ...
    params <- list("token" = app,
                   "user" = user,
                   "message" = message,
                   "attachment" = httr::upload_file(ftmp),
                   "priority" = priority,
                   "retry" = retry,
                   "expire" = expire,
                   "filename" = ftmp,
